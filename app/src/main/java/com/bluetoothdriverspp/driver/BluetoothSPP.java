...
public class BluetoothSPP {
    ...
    private static OnDataReceivedListener mDataReceivedListener = null;
    ...
    public static void setOnDataReceivedListener(OnDataReceivedListener listener) {
    ...
    public void send(String data, @SuppressWarnings("SameParameterValue") boolean CRLF) {
        if (mChatService.getState() == BluetoothState.STATE_CONNECTED) {
            if (CRLF)
                data += "\r\n";
    ...
    public void autoConnect(String keywordName) {
        if (!isAutoConnectionEnabled) {
            ...
            for (int i = 0; i < arr_name.length; i++) {
                if (arr_address[i].contains(keywordName)) {
